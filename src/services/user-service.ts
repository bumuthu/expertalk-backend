...
import { AuthenticationError, BusinessReject, DataNotFoundError, InternalServerError, KnownError, ValidationError } from "../utils/exceptions";
import { EntityService } from "./entity.service";
import { AuthType, BillingPeriod, PricingPlanName } from "../models/common";
import SysConfigDBModel from "../models/db/sys-config.model";
import { egress } from "../models/egress";
import { PricingService } from "@mintoven/common";
...
export class UserService extends EntityService {
    ...
    async getUserByUserId(userId: string): Promise<entity.User> {
        await this.before();

        const user: entity.User = await UserDBModel.findOne({ _id: userId });
        if (!user) throw new DataNotFoundError("User not found in the system");
    ...
    async createNewUser(newUser: ingress.SignUpInput): Promise<entity.User> {
        try {
            ...
            const checkExistingUsersWIthEmail: entity.User | null = await UserDBModel.findOne({ email: newUser.email });
            if (checkExistingUsersWIthEmail != undefined) {
                throw new ValidationError("User with the given email exists")
            }
            const userEntry: entity.User = await this.insertNewUser(newUser);
    ...
    async insertNewUser(newUser: ingress.SignUpInput): Promise<entity.User> {
        await this.before();
        ...
        const newUserDB: entity.User = {
            name: newUser.name,
            email: newUser.email,
            currentPlan: PricingPlanName.FREE,
            authType: AuthType.EMAIL,
            collectionAttemptsUsed: 0,
            billingStartsAt: timestamp,
            billingEndsAt: timestamp + MONTH_IN_MS,
            notifications: [{
                timestamp,
                title: `Welcome to Mintoon!`,
                description: `Welcome to Mintoon NFT journey. You have recieved your free tier rewards. Let's create NFT collections together.`
            }],
            history: [{
                timestamp,
                eventType: entity.HistoryEventType.GENERAL_EVENT,
                title: `Journey started`,
                description: `Mintoon account was created.`
            }],
            paymentHistory: []
        } as entity.User;
    ...
    async deleteUser(userId: string): Promise<entity.User> {
        try {
            await this.before();

            return UserDBModel.findByIdAndRemove(userId);
        } catch (e) {
            console.error(e);
            if (e.knownError) throw new KnownError(e.status, e.code, e.message);
            throw new InternalServerError();
    ...
    async upgradePricing(userId: string): Promise<entity.User> {
        try {
            await this.before();
    ...
    async validateUsersBillingPeriod() {
        ...
        try {
            ...
            const users: entity.User[] = await UserDBModel.find();
            const timestamp = Date.now();
